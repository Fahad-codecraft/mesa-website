    ...
    <>
      <div className="relative w-full">
        ...
        >
          <div
            className={cn(
              "absolute bottom-0 right-0 z-[1000] h-[5%] w-auto overflow-hidden bg-gradient-to-t"
            )}
          ></div>
          ...
          <div
            className={cn(
              "grid gap-4 pl-4",
              "max-w-7xl mx-auto",
              "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center justify-center" // Adjust the number of items per row based on screen size
            )}
          >
...
export const Card = ({
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <div className="fixed py-14 inset-0 h-screen z-50 overflow-auto">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              <motion.p
                ...
                className="text-3xl md:text-7xl text-center font-bold text-white"
              >
                {card.category}
              </motion.p>
              <motion.p
                layoutId={layout ? `title-${card.title}` : undefined}
                className="text-2xl font-semibold mt-4 text-center text-white"
              >
                {card.title}
              </motion.p>
